...
class Responsive {
  ...
  double getFontSizeByDeviceWidth(double fontSize) {
    ...
    if (width > _smallWidth && width < _mediumWidth) return fontSize.clamp(smallFontSize, mediumFontSize);
    return fontSize;
  }

  Widget spaceSize(double porcentHeight, BuildContext context) =>
      SizedBox(height: Responsive.of(context).heightPercent(porcentHeight));
}
